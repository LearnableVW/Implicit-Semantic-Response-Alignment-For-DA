...
def groupby_mean(value:torch.Tensor, labels:torch.LongTensor):
...
def centroid_update(loader, model, k):
    while True:
        start_test = True
        with torch.no_grad():
            iter_test = iter(loader["source"])
        ...
        centroids, k_labels = groupby_mean(all_embeddings, all_label)
        if k_labels.size()[0] == k:
            break

    return centroids, k_labels

def centroid_update_with_t(loader, model, k):
    while True:
        start_test = True
        with torch.no_grad():
            iter_test = iter(loader["source"])
            ...
            for i in range(len(loader['target'])):
                ...
                #labels = labels.cuda()
                inputs = inputs.cuda()
                _, outputs_target, embeddings, _ = model(inputs)

                labels = outputs_target.max(1).indices
                all_embeddings = torch.cat((all_embeddings, embeddings), 0)
                all_label = torch.cat((all_label, labels), 0)
...
def train(args):
    ...
    for i in range(args.max_iterations + 1):
        ...
        # update class embedding centroid
        if (i % len(dset_loaders["source"])== 0)  and  (i>1):
            centroids, c_labels = centroid_update_with_t(dset_loaders, base_network, args.class_num)
        ...
        #print(outputs_source)

        #print('source:', labels_source.size())
        #print('target:', outputs_target.max(1).indices.size())
        # print(outputs_target.max(1).indices)
        # print((F.softmax(outputs_target)).max(1).indices)
        # print((torch.nn.LogSoftmax()(outputs_target)).max(1).indices)        
        ...
        if centroids_exist:
            batch_cents = centroids[labels_source]
            ctrd_loss = F.mse_loss(e_s, batch_cents)
            total_loss += ctrd_loss * args.ctr_weight
            # print("centroids: ", ctrd_loss.item() * args.ctr_weight)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ctr_weight', type=float, default=20.0)
